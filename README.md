# Асинхронный сервис процессинга платежей (DDD)

## Описание
___
Микросервис для асинхронной обработки платежей, разработанный с использованием паттерна DDD (Domain-Driven Design):
...
- `GET /api/v1/payments/{payment_id}` - Получение данных указанного платежа:
  - статический API ключ в заголовке `X-API-Key`
- `POST /webhooks` - Обработка webhook уведомления
  - эмулирует обработку webhook уведомления (90% успех, 10% ошибка)
...
```shell
cd /opt/test-payment-processing-service-ddd
docker compose -f docker-compose.db-update.yml up -d && docker logs -f payment-processing-ddd-db-update
